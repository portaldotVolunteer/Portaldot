...
pub mod pallet {
	...
	/// The maximum active bond to become and maintain the role of a validator.
	#[pallet::storage]
	...
	impl<T: Config> Default for GenesisConfig<T> {
		fn default() -> Self {
			GenesisConfig {
				...
				max_validator_bond: Default::default(),
			}
		}
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn bond_extra(
			...
			if let Some(extra) = stash_balance.checked_sub(&ledger.total) {
				...
				if <Validators<T>>::contains_key(&ledger.stash) {
					...
					ensure!(ledger.active <= MaxValidatorBond::<T>::get(), Error::<T>::InsufficientBond);
				}
		...
		pub fn update_staking_limits(
			...
			max_validator_bond: BalanceOf<T>,
			...
			MaxValidatorBond::<T>::set(max_validator_bond);
...
impl<T: Config> Pallet<T> {
	...
	fn do_payout_stakers(validator_stash: T::AccountId, era: EraIndex) -> DispatchResultWithPostInfo {
		...
		let era_staked = Self::eras_total_stake(&era);
		...
		// nominators will get.
		let p = T::Currency::minimum_balance();
		let validator_total_reward_part = Perbill::from_rational(
			(BalanceOf::<T>::from(validator_reward_points)) * p + exposure.total,
			(BalanceOf::<T>::from(total_reward_points)) * p + era_staked,
		);
