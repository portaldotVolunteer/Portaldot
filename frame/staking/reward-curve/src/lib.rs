...
fn compute_points(input: &INposInput) -> Vec<(u32, u32)> {
	...
	let length = inpos.x_ideal / input.max_piece_count;

	let mut x = length;
	while x < inpos.x_ideal {
		let z: f64 = (x as f64/ 1_000_000.0).powi(2) - (x as f64 / 1_000_000.0).powi(3);
		// dbg!(&z);
		let y = (2_000_000.0 * z) as i128;
		// dbg!(&y);
		let y = y + x as i128;
		points.push((x, y as u32));
		x = x + length;
	}

	points.push((inpos.x_ideal, 3 * inpos.x_ideal as u32 / 2));
	
	let p = 1 as u32;
	points.push((1_000_000, (1_500_000 - (1_000_000 - inpos.x_ideal)/(2 * p)) as u32));
